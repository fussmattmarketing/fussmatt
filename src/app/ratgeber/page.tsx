...
export default async function RatgeberIndexPage() {
  ...
  // Convert static articles to unified type
  const staticArticles: ArticleCard[] = RATGEBER_ARTICLES.map((a) => ({
    ...a,
    source: "static" as const,
  }));

  // Convert WP posts to unified type
  const wpArticles: ArticleCard[] = wpPosts.map((p: BlogPost) => ({
    slug: p.slug,
    title: p.title,
    excerpt: p.excerpt,
    image: p.image,
  ...
  // Filter out WP posts that have same slug as static articles
  const staticSlugs = new Set(staticArticles.map((a) => a.slug));
  const uniqueWP = wpArticles.filter((a) => !staticSlugs.has(a.slug));
  const allArticles = [...staticArticles, ...uniqueWP].sort(
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-gray-900 to-gray-800 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
          <h1 className="text-3xl lg:text-4xl font-bold">Ratgeber & Blog</h1>
